...
class Independencies(object):
    ...
    def get_assertions(self):
        ...
        Examples
        --------
        >>> from ProbabilityModel.independencies import Independencies
        >>> independencies = Independencies(['X', 'Y', 'Z'])
    ...
    def add_assertions(self, *assertions):
        ...
        for assertion in assertions:
            if isinstance(assertion, IndependenceAssertion):
                event1, event2, event3 = assertion.get_assertion()
                for ev2 in event2:
                    self.independencies.append(
                        IndependenceAssertion(event1, [ev2], event3)
                    )

                # self.independencies.append(assertion)
            else:
                try:
                    if isinstance(assertion[1], str):
                        self.independencies.append(
                            IndependenceAssertion(assertion[0], assertion[1], assertion[2])
                        )
                    else:
                        for ev2 in assertion[1]:
                            self.independencies.append(
                                IndependenceAssertion(assertion[0], [ev2], assertion[2])
                            )
                except IndexError:
                    if isinstance(assertion[1], str):
                        self.independencies.append(
                        ...
                        for ev2 in assertion[1]:
                            self.independencies.append(
                                IndependenceAssertion(assertion[0], [ev2])
                            )
...
class IndependenceAssertion(object):
    ...
    Examples
    --------
    >>> from ProbabilityModel.independencies import IndependenceAssertion
