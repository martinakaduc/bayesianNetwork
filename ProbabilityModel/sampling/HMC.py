...
from ProbabilityModel.sampling import (
    LeapFrog,
...
class HamiltonianMC(object):
    ...
    Parameters
    ----------
    model: An instance ProbabilityModel.models
    ...
    simulate_dynamics: A subclass of ProbabilityModel.inference.continuous.BaseSimulateHamiltonianDynamics
        A class to propose future values of momentum and position in time by simulating
        Hamiltonian Dynamics

    Example
    -------
    >>> from ProbabilityModel.sampling import HamiltonianMC as HMC, LeapFrog, GradLogPDFGaussian
    ...
    def __init__(self, model, grad_log_pdf, simulate_dynamics=LeapFrog):
        ...
        if not issubclass(grad_log_pdf, BaseGradLogPDF):
            raise TypeError(
                ...
                + "ProbabilityModel.inference.base_continuous.BaseGradLogPDF"
            )

        if not issubclass(simulate_dynamics, BaseSimulateHamiltonianDynamics):
            raise TypeError(
                "split_time must be an instance of "
                + "ProbabilityModel.inference.base_continuous.BaseSimulateHamiltonianDynamics"
            )
...
class HamiltonianMCDA(HamiltonianMC):
    ...
    grad_log_pdf: A subclass of ProbabilityModel.inference.continuous.GradientLogPDF
    ...
    Example
    -------
    >>> from ProbabilityModel.sampling import HamiltonianMCDA as HMCda, LeapFrog, GradLogPDFGaussian as GLPG
    >>> from ProbabilityModel.factors.continuous import GaussianDistribution as JGD
    >>> import numpy as np
    >>> mean = np.array([1, 2, 3])
