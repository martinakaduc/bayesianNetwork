...
class BaseDiscretizer(ABCMeta):
    """
    Base class for the discretizer classes in ProbabilityModel. The discretizer
    ...
    >>> from ProbabilityModel.factors.continuous import ContinuousNode
    >>> normal = ContinuousNode(norm(0, 1).pdf)
    >>> from ProbabilityModel.discretize import BaseDiscretizer
    >>> class ChildDiscretizer(BaseDiscretizer):
    ...     def get_discrete_values(self):
    ...         pass
    >>> discretizer = ChildDiscretizer(normal, -3, 3, 10)
    >>> discretizer.factor
    <ProbabilityModel.factors.continuous.ContinuousNode.ContinuousNode object at 0x04C98190>
    ...
    def get_labels(self):
        ...
        Examples
        --------
        >>> from ProbabilityModel.factors import ContinuousNode
        >>> from ProbabilityModel.discretize import BaseDiscretizer
        >>> class ChildDiscretizer(BaseDiscretizer):
        ...     def get_discrete_values(self):
        ...         pass
...
class RoundingDiscretizer(BaseDiscretizer):
    ...
    >>> import numpy as np
    >>> from ProbabilityModel.factors.continuous import ContinuousNode
    >>> from ProbabilityModel.factors.continuous import RoundingDiscretizer
...
class UnbiasedDiscretizer(BaseDiscretizer):
    ...
    >>> import numpy as np
    >>> from ProbabilityModel.factors import ContinuousNode
    >>> from ProbabilityModel.factors.continuous import UnbiasedDiscretizer
